...
var RouterRegister *ControllerRegistor
// 是否模板渲染
var AutoRender bool
// 设置静态路由
...
type controllerInfo struct {
	...
	controllerMethod 	string			// 调用对应的方法
}
...
// 添加路由
func (p *ControllerRegistor) Add(pattern string, c controller.ControllerInterface, controllerMethod string) {
...
func (p *ControllerRegistor) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, route := range p.routers {
		...
		input[0] = reflect.ValueOf(ct)
		input[1] = reflect.ValueOf(route.controllerType.Name())
		init.Call(input)  // 调用初始化方法
		input = make([]reflect.Value, 0)
		method := vc.MethodByName("Prepare")  // 调用 Controller 的前期参数处理方法
		method.Call(input)
		// 调用对应的方法
		method = vc.MethodByName(route.controllerMethod)
		method.Call(input)
		// 渲染模板
		if AutoRender {
